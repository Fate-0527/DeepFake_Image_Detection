...
class Config:
    ...
    Vaild_FAKE_DIRS = {
        ...
        "/data1/DeepFake/FAKE_JPG/flux_dev": (570, 684),
    ...
    USE_MASTER_REPLICA = True      # 초기 성능 확인 시 False, 강건성 테스트 시 True
    COMPRESSION_PROB = 0.5         # 훈련 중 JPEG 압축을 적용할 확률
    JPEG_QUALITY_RANGE = (30, 100) # JPEG 압축 품질 범위

    # 4. 하이퍼파라미터
    R_MIN = 20                    # PSE 계산 시 최소 반지름
    R_MAX = 200                   # PSE 계산 시 최대 반지름
    T_STEP = 120                    # LaRE 노이즈 예측 시 사용되는 타임스텝
    IMG_SIZE = 512                 # 입력 이미지 크기
    BATCH_SIZE = 16          # GPU 메모리에 따라 조절 (8, 16, 32 등)
    NUM_EPOCHS = 100                # 훈련 에포크 수
    LEARNING_RATE = 1e-4        # 분류기 학습률
    NUM_WORKERS = 4                # 데이터 로딩에 사용할 CPU 프로세스 수

    # 5. 저장 및 기타
    SAVE_DIR = "/data1/checkpoints/HYRE"      # 모델 체크포인트 저장 경로
    PNG_DIR = "./result_png"        # 결과 PNG 이미지 저장 경로
    PNG_FILE = "training_8.png"  # 결과 PNG 파일 이름
    RANDOM_SEED = 42               # 재현성을 위한 시드값
    FILE_NAME = "mymodel8_bin"        # 저장되는 모델 파일 이름 접두사
